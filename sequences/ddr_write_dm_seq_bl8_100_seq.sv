//------------------------------------------------------------------------------
// Component    : ddr_write_dm_seq_bl8_100_seq
// Test Owner   : saketh
//
// Description  :
//   DDR WRITE with Data Mask (DM) sequence for Burst Length = 8.
//
//   Sequence Flow:
//     1. IDLE
//     2. Mode Register Set (BL=8, CL=2)
//     3. ACTIVATE
//     4. WRITE (with DM support)
//     5. READ
//     6. PRECHARGE
//     7. Second ACTIVATE → WRITE → READ (optional repeat flow)
//
//   This sequence verifies data masking behavior during WRITE bursts and
//   ensures correct READ-back from stored column addresses.
//------------------------------------------------------------------------------

class ddr_write_dm_seq_bl8_100_seq extends uvm_sequence #(DDR_seq_item);

  `uvm_object_utils(ddr_write_dm_seq_bl8_100_seq)

  //--------------------------------------------------------------------------
  // Sequence Variables
  //--------------------------------------------------------------------------
  DDR_seq_item seq;

  rand bit [1:0]  target_ba;
  rand bit [13:0] target_row;

  bit [9:0] col_addr[$];
  bit [9:0] pop_addr;

  //--------------------------------------------------------------------------
  // Constructor
  //--------------------------------------------------------------------------
  function new(string name = "ddr_write_dm_seq_bl8_100_seq");
    super.new(name);
  endfunction

  //--------------------------------------------------------------------------
  // Sequence Body
  //--------------------------------------------------------------------------
  virtual task body();

    seq = DDR_seq_item::type_id::create("seq");
    `uvm_info("SEQ_START", "DATA MASK sequence", UVM_LOW)

    // IDLE
    start_item(seq);
    if (!seq.randomize() with { cmd == IDLE; })
      `uvm_error("SEQ_IDLE", "random failed")
    finish_item(seq);

    // MRS
    start_item(seq);
    if (!seq.randomize() with { cmd == MRS; cfg_bl == 8; cfg_cl == 2; cfg_bt == 0; })
      `uvm_error("SEQ_MRS", "random failed")
    finish_item(seq);

    `uvm_info("SEQ_MRS",
              $sformatf("MRS Configured: BL=%0d CL=%0d",
                        seq.cfg_bl, seq.cfg_cl),
              UVM_LOW)

    // ACTIVATE
    start_item(seq);
    if (!seq.randomize() with { cmd == ACTIVATE; ba == target_ba; addr == target_row; })
      `uvm_error("SEQ_ACT", "random failed")
    finish_item(seq);

    `uvm_info("SEQ_ACT",
              $sformatf("=== BANK ACTIVATED === BA=%0d ROW=%0d",
                        seq.ba, seq.addr),
              UVM_LOW)

    // WRITE with DM
    start_item(seq);
    if (!seq.randomize() with {cmd== WRITE;cfg_bl== 8;ba== target_ba;addr[2:0]== 0;auto_precharge == 0;})
      `uvm_error("SEQ_WR", "random failed")

    seq.dm_mode = 1;
    col_addr.push_back(seq.addr);

    finish_item(seq);

    `uvm_info("SEQ_WR_DATA",
              $sformatf("WRITE -> BA=%0d COL_ADDR=%0d DM_MODE=%0d",seq.ba, seq.addr, seq.dm_mode),UVM_LOW)

    // READ
    seq = DDR_seq_item::type_id::create("seq");

    `uvm_info("SEQ_READ", "Starting Tracking Reads", UVM_LOW)

    if (col_addr.size() > 0) begin

      start_item(seq);

      pop_addr = col_addr.pop_front();

      if (!seq.randomize() with {cmd== READ;cfg_bl== 8;ba== target_ba;addr== pop_addr;auto_precharge == 0;})
        `uvm_error("SEQ_RD", "random failed")

      finish_item(seq);

      `uvm_info("SEQ_RD_DATA",
                $sformatf("READ -> BA=%0d COL_ADDR=%0d",
                          seq.ba, seq.addr),
                UVM_LOW)
    end

    // PRECHARGE
    start_item(seq);
    if (!seq.randomize() with { cmd == PRECHARGE; ba == target_ba; addr == target_row; })
      `uvm_error("SEQ_PRECHARGE", "random failed")
    finish_item(seq);

    `uvm_info("SEQ_PRE",$sformatf("=== BANK PRECHARGED === BA=%0d",target_ba),UVM_LOW)

    // SECOND ACTIVATE (optional repeat flow)
    start_item(seq);
    if (!seq.randomize() with { cmd == ACTIVATE; ba == target_ba; addr == target_row; })
      `uvm_error("SEQ_ACT2", "random failed")
    finish_item(seq);

    `uvm_info("SEQ_ACT",$sformatf("=== BANK ACTIVATED AGAIN === BA=%0d ROW=%0d",seq.ba, seq.addr),UVM_LOW)

    // SECOND WRITE
    start_item(seq);
    if (!seq.randomize() with {cmd== WRITE;cfg_bl== 8;ba== target_ba;addr[2:0]== 0;auto_precharge == 0;})
      `uvm_error("SEQ_WR2", "random failed")

    seq.dm_mode = 2;
    col_addr.push_back(seq.addr);

    finish_item(seq);

    // SECOND READ
    `uvm_info("SEQ_READ", "Starting Tracking Reads (2nd phase)", UVM_LOW)

    if (col_addr.size() > 0) begin

      start_item(seq);

      pop_addr = col_addr.pop_front();

      if (!seq.randomize() with {cmd== READ;cfg_bl== 8;ba== target_ba;addr== pop_addr;auto_precharge == 1;})
        `uvm_error("SEQ_RD2", "random failed")

      finish_item(seq);

      `uvm_info("SEQ_RD_DATA", $sformatf("READ -> BA=%0d COL_ADDR=%0d",seq.ba, seq.addr),UVM_LOW)
    end

  endtask

endclass
