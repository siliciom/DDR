// ----------------------------------------------------------------------------------------------------
// Component    : ddr_wr_auto_Pre_bfr_data_cmp_seq
// Test Owner   : Parikshith
//
// Description  :  Write read operations without maintaing delay sequence .
//
// Sequence Flow:
//   1. IDLE
...
//   5. READ
//
// This sequence performs  write read  operation without maintaining proper delay  to verify basic DDR functionality.
// --------------------------------------------------------------------------------------------------------



class ddr_wr_auto_pre_bfr_data_cmp_seq extends uvm_sequence #(DDR_seq_item);

  `uvm_object_utils(ddr_wr_auto_pre_bfr_data_cmp_seq)
  ...
  function new(string name = "ddr_wr_auto_pre_bfr_data_cmp_seq");
    super.new(name);
  endfunction
  ...
  task body();
    ...
    `uvm_info("SEQ_START", "Multiple write and read with randomized values", UVM_LOW)
    ...
    `uvm_info("SEQ_WR_DATA", $sformatf("WRITE BA=%0d COL=%0d AP=%0d",seq.ba, seq.addr, seq.auto_precharge),  UVM_LOW)
    ...
    if (col_addr.size() > 0) begin
      ...
      `uvm_info("SEQ_RD_DATA",$sformatf("READ BA=%0d COL=%0d AP=%0d",seq.ba, seq.addr, seq.auto_precharge), UVM_LOW)
    end
