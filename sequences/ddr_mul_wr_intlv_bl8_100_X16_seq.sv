// Component    : ddr_mul_wr_intlv_bl8_100_X16_seq
// Test Owner   : Anitha
//
...
class ddr_mul_wr_intlv_bl8_100_X16_seq extends uvm_sequence #(DDR_seq_item);

  `uvm_object_utils(ddr_mul_wr_intlv_bl8_100_X16_seq)
  ...
  function new(string name = "ddr_mul_wr_intlv_bl8_100_X16_seq");
    super.new(name);
  endfunction
  ...
  task body();
    ...
    `uvm_info("SEQ_IDLE", "IDLE command issued", UVM_LOW)
    ...
    repeat (2) begin
      ...
      // WRITE
      `uvm_info("SEQ_WRITE",$sformatf("Starting %0d WRITE transactions", num), UVM_LOW)
      ...
      // READ
      `uvm_info("SEQ_READ",$sformatf("Starting %0d READ transactions", num),UVM_LOW)
      ...
      if (!seq.randomize() with {
        ...
        `uvm_error("SEQ_PRECHARGE", "Randomization failed")

      finish_item(seq);

      `uvm_info("SEQ_PRE",$sformatf("PRECHARGE: BA=%0d",target_ba),UVM_LOW)
