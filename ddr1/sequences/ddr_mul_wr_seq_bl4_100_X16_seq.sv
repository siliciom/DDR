//=============================================================================
// Component    : ddr_mul_wr_seq_bl4_100_X16_seq
// Test Owner   : Harsh Sharma
//
// Description  :
//   Multiple DDR WRITE/READ sequence for Burst Length = 4 and x16 configuration.
//
// Sequence Flow:
//     1. Mode Register Set (BL=4, CL=2, Sequential Burst)
//     2. IDLE Command
...
class ddr_mul_wr_seq_bl4_100_X16_seq extends uvm_sequence #(DDR_seq_item);
  `uvm_object_utils(ddr_mul_wr_seq_bl4_100_X16_seq)
  ...
  // Constructor
  //---------------------------------------------------------------------------
  function new(string name = "ddr_mul_wr_seq_bl4_100_X16_seq");
    super.new(name);
  endfunction : new
  ...
  task body;
    ...
    repeat(2) begin
      ...
      repeat(num) begin
        start_item(seq);
        if(!seq.randomize with {cmd == WRITE; cfg_bl == 4; ba == target_ba; auto_precharge == 0; dm == 0; addr[2:0] == 0;}) begin
  ...
  endtask : body
endclass : ddr_mul_wr_seq_bl4_100_X16_seq
