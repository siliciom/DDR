// Component    : ddr_wr_ap_seq_bl4_100_seq
// Test Owner   : Siva Jyothi
//
// Description  :
//   Basic DDR WRITE/READ sequence with Auto Precharge enabled for READ,
//   using Sequential Burst Length = 4.
//
//   Sequence Flow:
//     1. IDLE
...
//     4. ACTIVATE
//     5. WRITE (Auto Precharge Disabled)
//     6. READ (Auto Precharge Enabled)
//
//   This sequence verifies READ Auto Precharge functionality after a
//   WRITE operation in Sequential Burst mode.
//

class ddr_wr_ap_seq_bl4_100_seq extends uvm_sequence #(DDR_seq_item);

  `uvm_object_utils(ddr_wr_ap_seq_bl4_100_seq)
  ...
  function new(string name ="ddr_wr_ap_seq_bl4_100_seq");
    super.new(name);
  endfunction
  ...
  task body();
    ...
    //-----------------------------------------------------------------------
    start_item(seq);
    assert(seq.randomize with {cmd == MRS; cfg_bl==4; cfg_cl==2; cfg_bt==0;});
    finish_item(seq);
    ...
    //-----------------------------------------------------------------------
    // READ Command (Auto Precharge Enabled)
    //-----------------------------------------------------------------------
    ...
    if (col_addr.size() > 0) begin
      ...
      if(!seq.randomize with {cmd == READ; cfg_bl == 4; ba == target_ba; addr == pop_addr; auto_precharge == 1;}) begin
